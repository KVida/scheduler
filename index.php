...
/*function time_start_or_end_watering($time_start, $duration , $flag_h_or_m) {
    if ($flag_h_or_m == 'm') {
        $arr_time_duration = explode(':', $duration);
        $time_m = $arr_time_duration[0]*1;
        $time_s = $arr_time_duration[1]*1;

        $dt = new DateTime($time_start);
        $dt->add(new DateInterval("PT" . $time_m . "M" . $time_s . "S"));
        $time_start_or_end = $dt->format('H:i:s');
    } else {
        $time_h = trim($duration, 'h');

        $dt = new DateTime($time_start);
        $dt->add(new DateInterval("PT" . $time_h . "H"));
        $time_start_or_end = $dt->format('H:i:s');
    }

    return $time_start_or_end;
}

while (true){
    for ($i=0; $i < $count_data; $i++) {
        $name_flower = $data[$i]["name"];
        $time_start = $data[$i]["start_At"];
        $duraction = $data[$i]["duraction"];
        $period = $data[$i]["period"];

        $time_end = time_start_or_end_watering($time_start, $duraction, 'm');
        ...
        if (($time_now == $time_start || $time_now > $time_start) && $time_now < $time_end) {
            ...
            $time_start =  time_start_or_end_watering($time_start, $data[$i]["period"], 'h');
            $data[$i]["start_At"] = $time_start;
            echo off($name_flower);
        }
    }
